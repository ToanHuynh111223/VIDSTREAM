...
const menuNavbar = [
  ...
  {
    name: "TV SHOWS",
    collection: ["Show Category", "Show Single", "Show Details"],
    isMenuChildren: true,
  },
  {
    name: "MOVIES",
    collection: ["Movies Category", "Movies Single", "Movies Details"],
    isMenuChildren: true,
  },
...
function ResponsiveNavbarHeader() {
  ...
  useEffect(() => {
    setListChildren(
      document.querySelectorAll(".Header_menuResponsiveChildren__9rvwe")
    );
  ...
  const onClickShowListChildren = (index) => {
    if (
      listChildren[index - 1].className ===
      "Header_menuResponsiveChildren__9rvwe Header_activeMenuChildren__5+gdA"
    ) {
      listChildren[index - 1].className = clsx(styles.menuResponsiveChildren);
    } else
      listChildren[index - 1].className = clsx(
        styles.menuResponsiveChildren,
        styles.activeMenuChildren
      );
      ...
      <div style={{ display: showNavbar ? "block" : "none" }}>
        <ul className={clsx(styles.listMenu)}>
          {menuNavbar.map((item, index) => {
            return item.isMenuChildren ? (
              <li
                className={clsx(styles.itemMenu)}
                key={index}
                onClick={() => onClickShowListChildren(index)}
              >
                ...
                <div className={clsx(styles.menuResponsiveChildren)}>
                  <ul className={clsx(styles.listMenuChildren)}>
                    {item.collection.map((collect, index) => {
                      return (
                        <li
                          className={clsx(styles.itemMenuChildren)}
                          key={index}
                        >
                          <Link
                            to={`/${collect.toLowerCase().replace(" ", "-")}`}
                          >
              ...
              <li className={clsx(styles.itemMenu)} key={index}>
                ...
                >
                  <div className={clsx(styles.itemContent)}>{item.name}</div>
                </Link>
              </li>
            );
          })}
